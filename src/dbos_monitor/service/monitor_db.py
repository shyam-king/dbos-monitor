...
class MonitorDB:
    ...
    async def upsert_executor(
        self, executor_id: str, executor_type: str, ping_interval_ms: int
    ) -> None:
        now_ms = _now_ms()
        async with self._pool.connection() as conn:
            await conn.execute(
    ...
    async def get_unhealthy_executors(
        self, grace_timeout_ms: int, unknown_timeout_ms: int
    ) -> list[ExecutorRecord]:
        now_ms = _now_ms()
        async with self._pool.connection() as conn:
            rows = await (
    ...
    async def get_healthy_executors_by_type(
        self, executor_type: str, grace_timeout_ms: int
    ) -> list[ExecutorRecord]:
        now_ms = _now_ms()
        async with self._pool.connection() as conn:
            rows = await (
